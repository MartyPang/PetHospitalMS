...
public class UserDao extends BaseDao {
	...
	public boolean addOperateLog(Map<String,Object> o){
		Connection conn = null;
		String sql = "insert into sc_opratelog(manager_id,name,operate_time,operate_ip,operate_url,operate,user_agent) values(?,?,?,?,?,?,?);";
		boolean result = false;
		try {
	...
	public boolean addAdmin(Map<String, Object> admin){
		...
		try{
			...
			sql = "insert into sc_admin(name,password,create_time,enable,authority) values(?,?,?,?,?)";
			result = getQueryRunner().update(conn, sql, name,password,create_time,1,authority)>0?true:false;
			DbUtils.closeQuietly(conn);
	...
	public boolean deleteAdmin(Integer id){
		...
		try{
			conn = DbControl.getConnection();
			sql = "update sc_admin set enable=0 where id=?";
	...
	public boolean editAdmin(Map<String, Object> admin){
		...
		try{
			...
			sql = "update sc_admin set name=?,password=?,authority=? where id=?";
			result = getQueryRunner().update(conn, sql, name,password,authority,id)>0?true:false;
			DbUtils.closeQuietly(conn);
	...
	public Map<String, Object> getUserList(int beginIndex, int pageSize) {
		...
		try {
			conn = DbControl.getConnection();
			// 获取管理员总数
			sql = "select count(*) from sc_admin where enable=1";
			totalCount = getQueryRunner().query(conn, sql,
					new ScalarHandler<Long>());
			// 获取管理员列表的一页数据
			sql = "select * from sc_admin where enable = 1 limit ?,?";
	...
	public Map<String, Object> getUserInfo(Integer id) {
		...
		try {
			conn = DbControl.getConnection();
			String sql = "select * from sc_admin where id=?";
			// 获取管理员详情信息
			userMap = getQueryRunner().query(conn, sql,
