...
public class ProcessDao extends BaseDao{
     ...
     * @return
     */
    public Map<String, Object> getProcessList(int beginIndex, int pageSize, int p_dmp){
        ...
        try{
            ...
            //获取部门数量
            sql = "select count(*) from ph_process where status=1 and p_dmp=?";
            totalCount = getQueryRunner().query(conn,sql,new ScalarHandler<Long>(),p_dmp);
            //获取一页
            sql = "select * from ph_process where status<>0 and p_dmp=? limit ?,?";
            list = getQueryRunner().query(conn,sql,new MapListHandler(new BasicRowProcessorFix()),p_dmp,beginIndex,pageSize);
    ...
    public boolean addProcess(Map<String, Object> process){
        ...
        try{
            ...
            Integer p_dmp = Integer.parseInt(process.get("p_dmp").toString());
            String p_link = createLink();
            String p_content = process.get("p_content").toString();
            String p_responsibility = process.get("p_responsibility").toString();
            String p_img = process.get("p_img").toString();
            String p_video = process.get("p_video").toString();
            Integer status = 1;
            String create_time = DateUtils.getCurrentDateTime();
            String update_time = create_time;

            sql = "insert into ph_process(p_name,p_dmp,p_link,p_content,p_responsibility,p_img,p_video,status,create_time,update_time) values(?,?,?,?,?,?,?,?,?,?)";

            result = getQueryRunner().update(conn, sql, p_name,p_dmp,p_link,p_content,p_responsibility,p_img,p_video,status,create_time,update_time)>0?true:false;
            DbUtils.closeQuietly(conn);
    ...
    public boolean editProcess(Map<String, Object> process){
        ...
        try{
            ...
            sql = "update ph_process set p_name=?,p_content=?,p_responsibility=?,p_img=?,p_video=?,update_time=? where p_id=?";
            result = getQueryRunner().update(conn, sql, p_name,p_content,p_responsibility,p_img,p_video,update_time,p_id)>0?true:false;
            DbUtils.closeQuietly(conn);
